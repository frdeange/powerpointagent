...
class SlideContent(BaseModel):
    ...
    image_prompt: str = Field(
        "", description="DALL-E prompt if an image should be generated"
    )
...
class DesignSpec(BaseModel):
    ...
    primary_color_hex: str = Field(
        "", description="Primary accent color in hex (e.g. #0078D4)"
    )
...
class ContentOutline(BaseModel):
    ...
    slides: list[SlideContent] = Field(
        default_factory=list, description="Per-slide content"
    )
...
class PresentationSpec(BaseModel):
    ...
    uploaded_document_url: str = Field(
        "", description="Blob URL of uploaded PPTX (optional)"
    )
    uploaded_document_blob: str = Field(
        "", description="Blob name in uploads container"
    )

    # Filled by each agent in sequence
    content_outline: ContentOutline | None = Field(
        None, description="Filled by ContentPlanner"
    )
    design_spec: DesignSpec | None = Field(
        None, description="Filled by DesignAgent or DocumentAnalyzer"
    )
    slides: list[SlideContent] = Field(
        default_factory=list, description="Filled by ContentWriter"
    )

    # Output
    presentation_id: str = Field(
        "", description="PPTX blob ID (filled by AssemblyAgent)"
    )
